...
def register():
    ...
    if flask.request.method == "POST":
        ...
        elif flask.request.form.get("password") != flask.request.form.get(
            "confirmation"
        ):
            return helpers.apology(
                "password and confirmation password must be same", 401
            )
        ...
        if user_check is None:
            user = User(
                username=flask.request.form.get("username"),
                hash=security.generate_password_hash(
            ...
            flask.flash(
                "Wow, we got a new user. You have successfully registered 😀", "success"
            )
...
def login():
    ...
    if flask.request.method == "POST":
        ...
        if user is None:
            ...
            user.hash, flask.request.form.get("password")
        ):
...
def logout():
    ...
    flask.flash(
        "You have successfully logged out. It's bad to see you go. We'll miss you 😔",
        "success",
    )
    return flask.redirect(flask.url_for("index"))
...
def quote():
    ...
    if flask.request.method == "POST":
        ...
        if quote is None:
            flask.flash(
                f"Couldn't get quote for \"{flask.request.form.get('symbol')}\". Please re-check spelling and try again.",
                "danger",
            )
...
def buy():
    ...
    if flask.request.method == "POST":
        ...
        if quote is None:
            flask.flash(
                f"Symbol \"{flask.request.form.get('symbol')}\" is invalid. Please re-check spelling and try again.",
                "danger",
            )
        # Else calculate required cash for successful purchase
            ...
            # Else flash message to inform user for insufficient funds
                ...
                flask.flash(
                    f"You don't have enough cash. Money required for purchase is {helpers.usd(cost)} and you have {helpers.usd(user_data.cash)} 😥",
                    "info",
                )

    # Trending stocks
    trending_stock_symbols = ["NFLX", "GOOGL", "AAPL", "AMZN", "ZNGA"]
    trending_stocks = []

    for stock_symbol in trending_stock_symbols:
        stock_data = helpers.lookup(stock_symbol)
        trending_stocks.append({"stock_symbol": stock_data["symbol"], "stock_price": stock_data["price"]})
    # Render template
    return flask.render_template("buy.html", trending_stocks=trending_stocks)
        ...
        for stock in portfolio:
            if flask.request.form.get("symbol") == stock["symbol"]:
                ...
                if int(flask.request.form.get("shares")) <= stock["shares"]:
                    ...
                    db.session.add(transaction)
                    user.cash = float(
                        user.cash + (price * int(flask.request.form.get("shares")))
                    )
                    db.session.commit()
